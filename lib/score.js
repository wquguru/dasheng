...
const MAX_QUESTIONS = 24; // 单次调用的问题上限；超出的替换词会明确标记为未判定
const MAX_SUBSTITUTIONS = Math.floor((MAX_QUESTIONS - 1) / 2); // 每个替换占两题，另留一题给达意评分
...
export async function scoreReading({ reference, heard, durationMs = 0, pauses = 0, askFn = ask }) {
  const a = align(reference, heard);
  const state = `原文:\n${reference}\n\n听到（ASR 已提交的文本）:\n${heard}`;

  const suspects = a.substitutions.slice(0, MAX_SUBSTITUTIONS);
  const unjudged = a.substitutions.slice(suspects.length).map((s) => ({
    refIndex: s.refIndex,
    word: s.ref.raw,
    heard: s.heard.raw,
  ...
  const reply = await askFn(state, questions);
  ...
  unjudged.forEach((item) => {
    const mark = marks[item.refIndex];
    if (!mark) return;
    mark.status = "doubt";
    mark.heard = item.heard;
    mark.unjudged = true;
  });

  const total = a.ref.length || 1;
  const spoken = a.matched.length + a.substitutions.length;
