...
import { paceScore, scoreReading, wpm } from "./score.js";
...
test("score caps Jev questions and reports unjudged substitutions as spoken", async () => {
  const reference = [];
  const heard = [];
  for (let i = 0; i < 13; i++) {
    reference.push(`word${i}`);
    heard.push(`other${i}`);
    if (i < 13) {
      reference.push(`bridge${i}`);
      heard.push(`bridge${i}`);
    }
  }
  reference.push("summer");
  ...
  const result = await scoreReading({
    ...
    heard: heard.join(" "),
    durationMs: 10000,
    askFn: async (_state, sentQuestions) => {
      questions = sentQuestions;
      return { model: "test", usage: {}, answers: { meaning: { score: 4 } } };
    },
  });

  assert.ok(Object.keys(questions).length <= 24);
  assert.equal(Object.keys(questions).length, 23);
  assert.equal(result.verdicts.length, 11);
  assert.equal(result.unjudged.length, 2);
  assert.deepEqual(
    result.unjudged.map(({ word, heard: recognized, reason }) => ({ word, heard: recognized, reason })),
    [
      { word: "word11", heard: "other11", reason: "question_limit" },
      { word: "word12", heard: "other12", reason: "question_limit" },
    ],
  );
  assert.equal(result.missed.length, 1);
  assert.equal(result.missed[0].word, "summer");
  assert.equal(result.sub.complete, 96);
  for (const item of result.unjudged) {
    assert.equal(result.marks[item.refIndex].status, "doubt");
    assert.equal(result.marks[item.refIndex].unjudged, true);
  }
});
